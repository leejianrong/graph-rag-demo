...
def test_creates_new_when_score_just_below_threshold() -> None:
    """A blocked candidate scoring just below the threshold → create-new."""
    store = InMemoryEntityStore()
    store.upsert(CanonicalEntity(canonical_id="apple", name=SURFACE, type="ORG", vector=_unit(0.8)))
    ...
    assert link.canonical_id != "apple"
    assert link.score < 0.85
    assert store.count() == 2  # the near-miss becomes its own canonical
...
def test_tiebreaker_off_makes_no_llm_call_near_threshold() -> None:
    ...
    The candidate scores 0.84 with threshold 0.85 and margin 0.05 — squarely in
    the band the tie-breaker would arbitrate if enabled. With the gate off the
    deterministic decision stands (create-new) and the LLM is never touched.
    """
    store = InMemoryEntityStore()
    store.upsert(
        CanonicalEntity(canonical_id="apple", name=SURFACE, type="ORG", vector=_unit(0.84))
    )
...
def test_tiebreaker_on_is_wired_and_fires() -> None:
    ...
    primary guarantee. The LLM says "yes" → the near-threshold decision merges.
    """
    store = InMemoryEntityStore()
    store.upsert(
        CanonicalEntity(canonical_id="apple", name=SURFACE, type="ORG", vector=_unit(0.84))
    ...
    assert result.links[0].canonical_id == "apple"
